...
class TestDockerCmdProcessorCall(TestDCPBase):
    def setup_method(self):
        ...
        self.processor._run_subprocess = MagicMock()
    ...
    @pytest.mark.parametrize("subcmd,expected_method", [
        ("up", "_create_up_cmd"),
        ("u", "_create_up_cmd"),
    ...
    def test_instance_call(self, subcmd, expected_method):
        ...
        getattr(self.processor, "_run_subprocess").assert_called_once()
    ...
    def test_call_dcpu(self):
        ...
        self.processor._run_subprocess.assert_called_once()
...
class TestRunSubprocess(TestDCPBase):
    def test_run_subprocess(self):
        ...
        self.processor._run_subprocess()

        subprocess.run.assert_called_once_with(test_cmd)
        assert self.processor._run_subprocess() == 1
...
class TestCreateOptions(TestDCPBase):
    ...
    @pytest.mark.parametrize("file_values,expected_value", [
        ([], []),
    ...
    @pytest.mark.parametrize("project_value,expected_value", [
        ([], []),
        (["fast-dcp"], ["-p", "fast-dcp"]),
    ])
